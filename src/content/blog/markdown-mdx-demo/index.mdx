...
To solve the cubic equation $t^3 + pt + q = 0$ (where the real numbers $p, q$ satisfy $4p^3 + 27q^2 > 0$) one can use Cardano's formula:
...
For any $u_1, \dots, u_n \in \mathbb\{C\} and $v_1, \dots, v_n \in \mathbb\{C\}, the Cauchy-Bunyakovsky-Schwarz inequality can be written as follows:
...
[Three famous mathematical formulas](https://developer.mozilla.org/en-US/docs/Learn/MathML/First_steps/Three_famous_mathematical_formulas) on MDN
...
<Callout variant="note" title="Click to expand and reveal hidden content">
  This is inside the collapsible section. Markdown elements like **bold text** and `code inline` should be styled correctly here.
</Callout>
